...
export const GET: RequestHandler = async ({ url }) => {
	...
	const filteredIngredients = langIngredients
		.filter((ingredient) => inputWords.every((word) => ingredient.toLowerCase().includes(word)))
	...
	return new Response(JSON.stringify(filteredIngredients.slice(0, 5)), { status: 200 });
};
