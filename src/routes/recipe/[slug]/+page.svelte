...
<div class="flex justify-center mt-5" in:fly|global={{ y: 100 }}>
	...
	>
		...
		{#if !isCurrentLanguage()}
			<button class="text-lg font-semibold pt-2" onclick={translateRecipe}>
				{$LL.recipe.translate()}
			</button>
		{/if}
		{#if !data.mealieUrl}
			<button class="text-lg font-semibold pt-2" onclick={uploadRecipe}>
				{$LL.recipe.upload()}
			</button>
		{:else}
